...
import com.example.notificationservice.services.pagination.paginationService;
...
public class SmsController {
    ...
    private  final paginationService paginationServices;
    ...
    @GetMapping(path = "/sms/search/{text}")
    public ResponseEntity<SmsResponseBody> searchGivenText(@PathVariable String text)  {
        try {
            return ResponseEntity.ok(paginationServices.searchText(text));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(SmsResponseBody.builder().error(e.getMessage()).build());
        }
    }
    ...
    @PostMapping(path = "/sms/between")
    public ResponseEntity<SmsResponseBody> getPhoneNumbersBetween(@RequestBody SmsRequestBody smsRequestBody) {
        try {
            return ResponseEntity.ok(paginationServices.getSmsBetweenDateAndTime(smsRequestBody));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(SmsResponseBody.builder().error(e.getMessage()).build());
        }
    }
